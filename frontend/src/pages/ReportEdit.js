...
function ReportEdit() {
  ...
  const loadReport = async () => {
    try {
      ...
      const report = response.data.find(r => r.report_id === reportId);
      ...
      setForm({
        ...
        violation_types: report.incident_details?.violation_types?.join(', ') || '',
        status: report.status || 'new',
        country: report.incident_details?.location?.country || '',
        region: report.incident_details?.location?.region || '',
        city: report.incident_details?.location?.city || '',
        address: report.incident_details?.location?.address || '',
        reporter_name: report.reporter_info?.name || '',
        reporter_contact: report.reporter_info?.contact || '',
      });

      if (report.incident_details?.location?.coordinates?.coordinates) {
        const coords = report.incident_details.location.coordinates.coordinates;
        setLocation({
          latitude: coords[1],
          longitude: coords[0],
          accuracy: report.incident_details.location.location_accuracy || null,
          error: null
        });
      }

      if (report.evidence) {
        setExistingEvidence(report.evidence);
      }
  ...
  const handleSubmit = async e => {
    ...
    const processedEvidence = [...existingEvidence]; 
  ...
  return (
    <div className="page form">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2>Edit Report: {form.report_id}</h2>
        <button 
          type="button" 
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="media-section">
          ...
          {existingEvidence.length > 0 && (
            <div>
              <h4>Existing Evidence ({existingEvidence.length}):</h4>
              {existingEvidence.map((evidence, index) => (
                <div key={index} style={{
                  ...
                  <div>
                    <strong>{evidence.description || 'Evidence file'}</strong><br/>
                    <small>Type: {evidence.type} | URL: {evidence.url}</small>
                  </div>
                  <button 
                    type="button"
          ...
          {}
          <label>Add New Files (Photos, Videos, Documents)</label>
          <input 
            type="file"
            multiple
          ...
          {mediaFiles.length > 0 && (
            <div className="media-files-list">
              <h4>New Files to Upload ({mediaFiles.length}):</h4>
              {mediaFiles.map(file => (
